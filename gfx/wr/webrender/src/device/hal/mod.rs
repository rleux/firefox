...
#[cfg(all(target_os = "linux", feature = "hal-linux-dmabuf"))]
pub use self::vulkan::{VideoDmaBufCapabilities, VideoDmaBufFormat, VideoDmaBufLayout, ForeignYuvImage, WeakForeignYuvImage};
#[cfg(all(target_os = "linux", feature = "hal-linux-dmabuf"))]
...
#[cfg(wr_hal_vulkan)]
pub fn create_vulkan_backend(options: &Options) -> Result<crate::device::GpuBackendConfig> {
    let device = create_vulkan_device(options)?;
    let backend = render::gpu_backend::HalGpuBackend::new(device)?;
    Ok(crate::device::GpuBackendConfig::Hal(Box::new(backend)))
}
