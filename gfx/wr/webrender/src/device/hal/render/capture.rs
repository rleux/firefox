...
impl<A: BackendApi> FrameRenderer<A> {
    ...
    #[cfg(feature = "replay")]
    pub(crate) fn validate_capture(&self, config: &CaptureConfig) -> Result<()> {
    ...
    #[cfg(feature = "capture")]
    pub(crate) fn begin_capture_metadata(&self, config: &CaptureConfig) -> Result<()> {
        let root = config.resource_root();
        fs::create_dir_all(&root).map_err(|e| e.to_string())?;
        fs::write(root.join("hal-identity.ron"), "capture-in-progress").map_err(|e| e.to_string())?;
        fs::write(root.join("hal-filtering.txt"), self.filtering.name()).map_err(|e| e.to_string())?;
        fs::write(root.join("hal-shader-input.txt"), self.shader_input.name()).map_err(|e| e.to_string())
    }

    #[cfg(feature = "capture")]
    pub(crate) fn finish_capture_metadata(&self, config: &CaptureConfig) -> Result<()> {
        write_ron(config.resource_root().join("hal-identity.ron"), &self.capture_identity())
    }
