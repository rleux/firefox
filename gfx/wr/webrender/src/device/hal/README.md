...
`create_vulkan_renderer` returns the HAL API facade and a normal `RenderApiSender`.
The facade uses the upstream `Renderer`, selected through `GpuBackendConfig::Hal`.
`HalGpuBackend` implements resource handles, uploads, render passes, pipeline state,
draws, fences and readback through the existing HAL resource and submission code.
Renderer initialization, scene building, preparation, batching, task processing
and texture-cache updates use the shared renderer.

HAL's Native and Layer compositor callbacks are adapted to WebRender's compositor
interfaces. Native surfaces retain their image leases through GPU completion.
The HAL facade preserves presentation, capture/replay, frame readiness and image
ownership APIs. Wrench's renderer-independent YAML client works with either
renderer; there is no GL context or GL fallback in the HAL route.
...
the original reference images. External images and Draw/Native/Layer targets retain their HAL import and
ownership contracts. Unsupported backend operations fail. Full Vulkan Wrench acceptance remains open.
...
The device adapter converts the renderer's clip coordinates to HAL's Y orientation
and depth range; partial render passes preserve pixels outside their render area.
