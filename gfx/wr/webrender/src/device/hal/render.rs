...
impl<A: BackendApi> FrameRenderer<A> {
    ...
    fn pipeline(&mut self, key: PipelineKey) -> Result<Rc<Pipeline<A>>> {
        ...
        let blend = match key.blend & 0x7f {
            0 => None,
            ...
            native.create_render_pipeline(&hal::RenderPipelineDescriptor {
                ...
                depth_stencil: if key.depth == 0 {
                    ...
                    Some(wgt::DepthStencilState {
                        ...
                        depth_write_enabled: Some(matches!(key.depth, 1 | 4 | 5)),
                        depth_compare: Some(match key.depth {
                            3 | 4 => wgt::CompareFunction::Always,
                            5 | 6 => wgt::CompareFunction::Less,
                            _ => wgt::CompareFunction::LessEqual,
                        }),
                ...
                color_targets: &[Some(wgt::ColorTargetState {
                    format: key.format,
                    blend,
                    write_mask: if key.blend & 0x80 == 0 { wgt::ColorWrites::ALL } else { wgt::ColorWrites::empty() },
                })],
                multiview_mask: None,
                cache: None,
            })
