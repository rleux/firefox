...
use crate::api::{
    Checkpoint, DocumentId, ImageFormat, NotificationRequest, RenderBackendId,
    RenderNotifier,
};
use crate::api::channel::{Receiver, Sender};
...
pub(crate) fn create_renderer_with_factory<A: BackendApi>(
    ...
    let (gpu, sender) = GpuRenderer::new(gpu, options, notifier)?;
    let renderer = gpu.inner.as_ref().unwrap();
    let result_rx = renderer.result_rx.clone();
    let api_tx = renderer.api_tx.clone();
    let backend_id = renderer.backend_id;
    let pool = renderer._render_backend_pool.clone();
...
impl<A: BackendApi> RendererCore<A> {
    ...
    fn flush_required_frame(&mut self) -> Result<(), String> {
        if let Some(document) = &mut self.document {
            if document.frame.must_be_drawn() {
                self.gpu.render_document(self.document_id.unwrap(), document, false, None)?;
                document.frame.has_been_rendered = true;
            }
        }
        for (&id, document) in self.parked_documents.iter_mut() {
            if document.frame.must_be_drawn() {
                self.gpu.render_document(id, document, false, None)?;
                document.frame.has_been_rendered = true;
            }
        }
        Ok(())
    ...
    fn process_message(&mut self, message: ResultMsg) -> Result<(), String> {
        ...
        match message {
            ...
            ResultMsg::RenderDocumentOffscreen(id, mut document, updates) => {
                ...
                self.flush_required_frame()?;
                self.apply_resources(updates)?;
                self.gpu.render_document(id, &mut document, false, None)?;
            }
            ...
            ResultMsg::AppendNotificationRequests(requests) => {
                for request in requests {
                    if request.when() == Checkpoint::FrameTexturesUpdated {
                        self.gpu.notify_texture_update(request);
                    } else {
                        self.notifications.push(request);
                    }
                }
            }
    ...
    fn execute_frame(&mut self) -> Result<RenderedFrame<A>, String> {
        ...
        self.gpu.inner.as_mut().unwrap().clear_color = self.clear_color;
        let output = self.gpu.render_document(self.document_id.unwrap(), document, document.frame.present, partial_damage)?;
        if let Some(metrics) = self.gpu.metrics() {
...
mod tests {
    ...
    fn storage_tables_match_texture_pixels_and_retire() {
        ...
        for phase in 0..4 {
            ...
            for use_buffers in [true, false] {
                ...
                assert!(output.stats.data_table_uploads <= 6);
                assert_eq!(output.stats.data_table_copies, if use_buffers { 0 } else { output.stats.data_table_uploads });
    ...
    #[test]
    #[ignore = "Requires Vulkan and validation"]
    fn texture_checkpoint_waits_for_uploads() {
        use crate::render_api::Transaction;
        use api::units::*;
        use api::*;
        ...
        transaction.add_image(
            ...
            ImageData::new(vec![0, 0, 255, 255].repeat(256)),
            None,
        );
        transaction.set_root_pipeline(pipeline);
        transaction.set_display_list(Epoch(0), api.get_namespace_id(), builder.end());
        ...
        assert!(
            checkpoints.lock().unwrap().is_empty(),
            "Texture checkpoint fired before pending uploads"
        );
        let output = renderer.render_frame().unwrap();
        assert_eq!(
