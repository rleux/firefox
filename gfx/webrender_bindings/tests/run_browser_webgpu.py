...
def main():
    ...
    parser.add_argument("--binary", type=Path, default=root / "obj-x86_64-pc-linux-gnu/dist/bin/firefox")
    ...
    env = os.environ.copy()
    for name in ["MOZ_WR_BACKEND", "MOZ_WR_DEFAULT_BACKEND", "MOZ_RUN_GTEST", "MOZ_HEADLESS",
                 "MOZ_WR_VULKAN_VALIDATION", "VK_LAYER_PATH", "VK_INSTANCE_LAYERS", "VK_LAYER_VALIDATE_SYNC"]:
        env.pop(name, None)
    env.update(
        WR_WEBGPU_OUTPUT=str(output), WR_WEBGPU_TRANSPORT=args.transport,
        ...
        GDK_BACKEND="x11", WGPU_VALIDATION="1" if args.validation_layers else "0",
    ...
    if args.validation_layers:
        env.update(MOZ_WR_VULKAN_VALIDATION="1", VK_LAYER_PATH=str(args.validation_layers.resolve()), VK_LAYER_VALIDATE_SYNC="1")
    if args.adapter:
    ...
    prefs = {
        ...
        "gfx.webrender.all": "true", "gfx.webrender.vulkan": "true",
        "gfx.webrender.software": "false", "gfx.color_management.mode": "0",
        "layout.css.devPixelsPerPx": "1.0", "layers.gpu-process.enabled": args.gpu_process,
        "dom.webgpu.enabled": "true", "dom.webgpu.force-enabled": "true",
        "dom.webgpu.allow-present-without-readback": "true", "dom.webgpu.hal-labels": "true",
    }
    if args.benchmark_frames:
        prefs["privacy.reduceTimerPrecision"] = "false"
    command = [str(root / "mach"), "marionette-test", "--binary", str(args.binary.resolve()),
               "--gecko-log", str(output / "gecko.log"), "--workspace", str(output / "workspace"),
               "--startup-timeout", "90", "--socket-timeout", "60"]
    for name, value in prefs.items():
        command += ["--setpref", f"{name}={value}"]
    command.append(str(Path(__file__).with_name("test_webgpu_dmabuf_browser.py")))
    print(f"Output: {output}", flush=True)
    with (output / "test.log").open("w") as log, (output / "window-manager.log").open("w") as wm_log:
        wm = subprocess.Popen(["openbox", "--sm-disable"], env=env, stdout=wm_log, stderr=subprocess.STDOUT)
        try:
